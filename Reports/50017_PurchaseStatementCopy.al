report 50017 "Purchase Statement copy"
{
    // version 
    DefaultLayout = RDLC;
    ...
    dataset
    {
        ...
        Vendor)
        {
            ...
            "Vendor Ledger Entry")
            {
                ...
                dataitem("Detailed Vendor Ledg. Entry";
                "Detailed Vendor Ledg. Entry")
                {
                    DataItemLink = "Applied Vend. Ledger Entry No." = FIELD("Entry No.");
                    DataItemTableView = SORTING("Applied Vend. Ledger Entry No.", "Entry Type") WHERE("Entry Type" = CONST(Application), "Document Type" = FILTER(<> Refund), Unapplied = CONST(false));
                    ...
                    begin
                        ...
                        VoidVLE.SETRANGE(VoidVLE."Source Code", 'FINVOIDCHK');
                        IF VoidVLE.FINDFIRST THEN CurrReport.SKIP;
                        VoidVLE.RESET;
                        VoidVLE.SETRANGE("Entry No.", "Detailed Vendor Ledg. Entry"."Applied Vend. Ledger Entry No.");
                        VoidVLE.SETRANGE(VoidVLE."Source Code", 'FINVOIDCHK');
                        IF VoidVLE.FINDFIRST THEN CurrReport.SKIP;
                        IF "Detailed Vendor Ledg. Entry"."Document Type" = "Detailed Vendor Ledg. Entry"."Document Type"::" " THEN IF "Detailed Vendor Ledg. Entry"."Source Code" = 'FINVOIDCHK' THEN CurrReport.SKIP;
                        IF "Detailed Vendor Ledg. Entry"."Document Type" = "Detailed Vendor Ledg. Entry"."Document Type"::" " THEN IF "Detailed Vendor Ledg. Entry"."Source Code" = 'PURCHAPPL' THEN IF VLE.GET("Detailed Vendor Ledg. Entry"."Applied Vend. Ledger Entry No.") THEN IF VLE."Source Code" = 'FINVOIDCHK' THEN CurrReport.SKIP;
                        IF ("Detailed Vendor Ledg. Entry"."Document Type" = "Detailed Vendor Ledg. Entry"."Document Type"::" ") AND ("Detailed Vendor Ledg. Entry"."Initial Document Type" = "Detailed Vendor Ledg. Entry"."Initial Document Type"::" ") THEN IF "Detailed Vendor Ledg. Entry"."Source Code" = 'PAYMENTJNL' THEN IF VLE.GET("Detailed Vendor Ledg. Entry"."Applied Vend. Ledger Entry No.") THEN IF VLE."Source Code" = 'PAYMENTJNL' THEN CurrReport.SKIP;
                        VLE.Reset();
                        VLE.SetRange("Entry No.", "Detailed Vendor Ledg. Entry"."Vendor Ledger Entry No.");
                        VLE.SetRange("Posting Date", StartingDate, EndingDate);
                        if not VLE.FindFirst() then CurrReport.Skip();
                        /*  VLE.Reset();
                                             VLE.SetRange("Entry No.", "Detailed Vendor Ledg. Entry"."Vendor Ledger Entry No.");
                                ...
                                IF "Detailed Vendor Ledg. Entry"."Entry Type" = "Detailed Vendor Ledg. Entry"."Entry Type"::Application THEN BEGIN
                                    IF ("Detailed Vendor Ledg. Entry"."Document Type" IN ["Detailed Vendor Ledg. Entry"."Document Type"::Invoice, "Detailed Vendor Ledg. Entry"."Document Type"::"Credit Memo"]) THEN BEGIN
                                        WITH DetailedVendorLedgEntry DO BEGIN
                                            ...
                                            IF FIND('-') THEN
                                                REPEAT
                                                    IF "Entry No." > ApplicationEntryNo THEN ApplicationEntryNo := "Vendor Ledger Entry No.";
                                                    //Test 2nd Sceaniro
                                                    ...
                                                    IF DetailedVendorLedgEntry2.FINDFIRST THEN SkipRecord := FALSE;
                                                UNTIL NEXT = 0;
                                            IF SkipRecord THEN CurrReport.SKIP;
                                        END;
                                    END;
                        ...
                        IF (VLE.GET("Detailed Vendor Ledg. Entry"."Vendor Ledger Entry No.")) THEN BEGIN
                            ...
                            IF (VLE."Document Type" <> VLE."Document Type"::Payment) OR ((VLE."Document Type" = VLE."Document Type"::Payment) AND ("Detailed Vendor Ledg. Entry"."Initial Document Type" <> "Detailed Vendor Ledg. Entry"."Initial Document Type"::Payment)) THEN BEGIN
                    ...
                    begin
                        IF COUNTI > 1 THEN IF VarGrossAmtPaiddup = VarGrossAmtPaid THEN CurrReport.SKIP;
                ...
                "Detailed Vendor Ledg. Entry")
                {
                    ...
                    DataItemTableView = SORTING("Applied Vend. Ledger Entry No.", "Entry Type") WHERE("Entry Type" = CONST("Initial Entry"), "Document Type" = FILTER(Payment));
                    ...
                    begin
                        ...
                        VLE.CALCFIELDS(VLE."Original Amt. (LCY)", VLE."Remaining Amt. (LCY)");
                        IF (VLE."Remaining Amt. (LCY)" = 0) THEN CurrReport.SKIP;
                        VarDesc := DELCHR(VLE.Description);
                        //VarDesc := VLE.Description;
                        VarDesc2 := VLE.Description;
                        CLEAR(VarInvoiceNo);
    ...
    begin
        ...
        IF STRLEN(VarDayText) = 1 THEN VarDayText := '0' + VarDayText;
        VarMonthTxt := FORMAT(DATE2DMY(ParamDate, 2));
        IF STRLEN(VarMonthTxt) = 1 THEN VarMonthTxt := '0' + VarMonthTxt;
    ...
    begin
        ...
        ExcelBuf.AddColumn("Company Information"."Telephone Number - MRA", FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn("Company Information"."Mobile Number - MRA", FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn("Company Information"."Name of Declarant", FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.AddColumn("Company Information"."Email Address - MRA", FALSE, '', FALSE, FALSE, FALSE, '', ExcelBuf."Cell Type"::Text);
        ExcelBuf.NewRow;
    ...
    begin
        ExcelBuf.CreateBookAndOpenExcel('', 'Purchase Statement', '', COMPANYNAME, USERID);
    end;
    ...
    begin
        ...
        WITH DetailedVendorLedgEntry DO BEGIN
            ...
            IF DetailedVendorLedgEntryInitial.FINDFIRST THEN SETFILTER("Initial Document Type", '%1|%2', DetailedVendorLedgEntry."Initial Document Type"::Payment, DetailedVendorLedgEntry."Initial Document Type"::Invoice);
            //SETFILTER("Source Code",'<>%1','FINVOIDCHK');
            IF NOT FIND('-') THEN SkipRecord := TRUE
        END;
        IF NOT SkipRecord THEN EXIT(FALSE);
        DetailedVendorLedgEntry.RESET;
        WITH DetailedVendorLedgEntry DO BEGIN
            IF "Detailed Vendor Ledg. Entry"."Transaction No." = 0 THEN BEGIN
                SETCURRENTKEY("Application No.", "Vendor No.", "Entry Type");
        ...
        WITH DetailedVendorLedgEntry DO BEGIN
            ...
            IF FINDSET THEN
                REPEAT
                    IF PaymentVendorLedgerEntry.GET("Applied Vend. Ledger Entry No.") AND (PaymentVendorLedgerEntry."Bal. Account Type" = PaymentVendorLedgerEntry."Bal. Account Type"::"Bank Account") THEN SkipRecord := FALSE;
                UNTIL NEXT = 0;
        END;
        EXIT(SkipRecord);
    end;
