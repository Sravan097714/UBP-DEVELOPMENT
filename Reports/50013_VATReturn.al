report 50013 "VAT Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Reports\Layout\VATReport.rdl';
    Caption = 'RC-Consolidated VAT Report';
        ...
        dataitem("VAT Entry"; "VAT Entry")
        {
            ...
            begin
                ...
                IF Customer.FINDFIRST THEN BEGIN
                    ...
                    BRN := Customer."Telex No.";
                END;
                vendorrec.Reset();
                vendorrec.SetRange(vendorrec."No.", "VAT Entry"."Bill-to/Pay-to No.");
                if vendorrec.FindFirst then begin
                    CustomerName := vendorrec.Name;
                    VATRegNo := vendorrec."VAT Registration No.";
                    BRN := vendorrec."Telex No.";
                end;
                        ...
                        Type::Sale:
                            begin
                                if "Document Type" = "Document Type"::Invoice then begin
                                    if not SalesInvHeaderRec.Get("VAT Entry"."Document No.") then
                                        Clear(SalesInvHeaderRec);
                                    SalesDesc := SalesInvHeaderRec."Posting Description"
                                end else
                                    if "Document Type" = "Document Type"::"Credit Memo" then begin
                                        if not SalesCrdHdr.Get("VAT Entry"."Document No.") then
                                            Clear(SalesCrdHdr);
                                        SalesDesc := SalesCrdHdr."Posting Description";
                                    end;
                            end;
                        Type::Purchase:
                            begin
                                if "Document Type" = "Document Type"::Invoice then begin
                                    if not PurInvHdr.Get("VAT Entry"."Document No.") then
                                        Clear(PurInvHdr);
                                    SalesDesc := PurInvHdr."Posting Description"
                                end else
                                    if "Document Type" = "Document Type"::"Credit Memo" then begin
                                        if not PurCrdHdr.Get("VAT Entry"."Document No.") then
                                            Clear(PurCrdHdr);
                                        SalesDesc := PurCrdHdr."Posting Description";
                                    end;
                            end;
                    end
                end;
                If SalesDesc = '' then begin
                    GLEntry.Reset();
    ...
    var
        ...
        PurCrdHdr: Record "Purch. Cr. Memo Hdr.";
