...
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Reports\Layout\Order1.rdl';
    Caption = 'Order';
        ...
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(PreparedByCaptionLbl; PreparedByCaptionLbl)
                    {
                    }
                    column(AuthorisedByCaptionLbl; AuthorisedByCaptionLbl)
                    {
                    }
                    ...
                    column(AuthorisedDate; AuthorisedDate)
                    {
                    }
                    column(ComputerLbl; ComputerLbl)
                    {
                    }
                    column(Customer_BRNNo; Vendor."Telex No.")
                    {
                    }
                    column(Customer_BRNNoLbl; 'BRN No.')
                    {
                    }
                    column(CommentsText; Comments + Comments1)
                    {
                    }
                    column(BranchOrpertionLbl; BranchOrpertionLbl)
                    {
                    }
                    column(Purch_SD1; "Purchase Header"."Shortcut Dimension 1 Code")
                    {
                    }
                    ...
                    dataitem(RoundLoop; "Integer")
                    {
                        ...
                        column(VatPostingGroup; "Purchase Line"."VAT Prod. Posting Group")
                        {
                        }
                        ...
                        column(VatPostingGroup_PurchLineCaption; 'VAT Rate')
                        {
                        }
            ...
            begin
                ...
                if ApprovalEntry.FindLast() then begin
                    ...
                    AuthorisedBy := ApprovalEntry."Approver ID";
                    PreparedDate := DT2Date(ApprovalEntry."Date-Time Sent for Approval");
                    AuthorisedDate := DT2Date(ApprovalEntry."Last Date-Time Modified");
                end else begin
                ...
                PurchaseComments.SetRange("Document Type", PurchaseComments."Document Type"::Order);
                PurchaseComments.SetRange("No.", "No.");
                PurchaseComments.SetRange("Document Line No.", 0);
                if PurchaseComments.FindSet() then
                    repeat
                        if StrLen(Comments + PurchaseComments.Comment) < MaxStrLen(Comments) then
                            Comments += PurchaseComments.Comment
                        else
                            if StrLen(comments1 + PurchaseComments.Comment) < MaxStrLen(comments1) then
                                comments1 += PurchaseComments.Comment;
    ...
    var
        ...
        CompanyInfoGiroNoCaptionLbl: Label 'BRN No.';
        ...
        DateLbl: Label 'Date';
        ComputerLbl: Label 'This is computer generated document and needs no signature';
        PreparedBy: Text;
        AuthorisedBy: Text;
        PreparedDate: Date;
        AuthorisedDate: Date;
        ...
        Comments: Text;
        comments1: Text;
        BranchOrpertionLbl: Label 'Branch Operation';
        Test: Report "Standard Sales - Quote";
